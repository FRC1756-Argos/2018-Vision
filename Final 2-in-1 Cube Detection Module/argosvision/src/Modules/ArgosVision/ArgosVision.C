...
const uchar HMIN = 1;
const uchar HMAX = 180;
const uchar MATLAB_HMAX = 255;
    
class ArgosVision : public jevois::StdModule,
                    public jevois::Parameter< detect_type, hrange_powercube, srange_powercube, vrange_powercube, 
                                              maxnumobj, cube_area_range, cube_area, erodesize, dilatesize, debug>
{
  public:
    //!constructor
    ...
    cv::Mat detectPowerCubes(const cv::Mat &imghsv, jevois::RawImage &outimg);
    ...
    void process(jevois::InputFrame && inframe, jevois::OutputFrame && outframe) override
    {
      ...
      uint8_t detectionType = detect_type::get();

      //load only one time
      if(m_loadYml)
      {
        Pr_cube_given_pixel_equals_1 = getProbabilityDistribution("Pr_cube_given_pixel_equals_1", std::string(YML_FILE));
        Pr_cube_given_pixel_equals_0 = getProbabilityDistribution("Pr_cube_given_pixel_equals_0", std::string(YML_FILE));
        row = Pr_cube_given_pixel_equals_1.rows;
        col = Pr_cube_given_pixel_equals_1.cols;
        m_loadYml = false;
      }
      ...
      {
        case BAYESIAN:
            imgth = detectPowerCubes(imghsv, outimg);
            imgth.convertTo(imgth, CV_8UC1);
            break;
        case HSV:
      ...
      cv::findContours(imgth, contours, hierarchy, CV_RETR_TREE, CV_CHAIN_APPROX_NONE);
      ...
      jevois::rawimage::writeText(outimg, "Detected R " + std::to_string(row) +  " C: " + std::to_string(col), 3, m_h - 20, jevois::yuyv::White);
    ...
    cv::Mat Pr_cube_given_pixel_equals_1;
    cv::Mat Pr_cube_given_pixel_equals_0;
    //load yml file
    bool m_loadYml;
    bool m_loadedYml;
};
...
cv::Mat ArgosVision::getProbabilityDistribution(std::string data, std::string ymlfilename)
{
  ...
  cv::Mat temp;
  cv::FileStorage fs2(ymlfilename, cv::FileStorage::READ);
  fs2[data] >> temp;
  fs2.release();
  m_loadedYml = true;
  return temp;
...
////////////////////////////////////////////////////////////////////////////////
cv::Mat ArgosVision::detectPowerCubes(const cv::Mat &imghsv, jevois::RawImage &outimg)
{
  uchar Hue, Sat;
  cv::Mat Posterior_probability_map = cv::Mat(imghsv.rows, imghsv.cols, CV_8UC1, uchar(0));

  if(!m_loadedYml)
  {
    jevois::rawimage::writeText(outimg, "Unable to load YML file for probability distributions", 3, m_h - 40, jevois::yuyv::White);
    return Posterior_probability_map;
  }

  double pr_pixel_equals_0 = 0.9653;
  double pr_pixel_equals_1 = 1 - pr_pixel_equals_0;

  cv::Mat Pr_pixel_equals_1_given_cube = cv::Mat(imghsv.rows, imghsv.cols, CV_32FC1, float(0.0));
  ...
  for (int i = 0; i < imghsv.rows; ++i)
  {
      ...
      {
          Hue = pixel[j][0];
          //normalize to 1-255, as matlab uses the same
          Hue = floor((Hue - HMIN)*(MATLAB_HMAX/(HMAX - HMIN))) + 1;
          Sat = pixel[j][1] + 1;
          
          float pr_cube = ((Pr_cube_given_pixel_equals_1.at<float>(Hue, Sat) * pr_pixel_equals_1)
                          + (Pr_cube_given_pixel_equals_0.at<float>(Hue, Sat) * pr_pixel_equals_0));
                          
          //pr_cube = pr_cube + 0.1; //avoid divided by zero
  ...
  float abc = Pr_cube_given_pixel_equals_1.at<float>(Hue, Sat);
  jevois::rawimage::writeText(outimg, "Detected R " + std::to_string(abc) +  " C: " + std::to_string(Pr_cube_given_pixel_equals_1.cols) + "E " + std::to_string(imghsv.cols), 3, m_h - 40, jevois::yuyv::White);
...
void ArgosVision::trackPowerCubes(const std::vector<std::vector<cv::Point>> &contours, const std::vector<cv::Vec4i> &hierarchy, jevois::RawImage &outimg)
{
  ...
  if (hierarchy.size() > 0 && hierarchy.size() <= maxnumobj::get())
  {
    ...
    for (int index = 0; index >= 0; index = hierarchy[index][0])
    {
      ...
      if (cube_area::get().contains(int(area + 0.4999)) && area > cubeArea )
      { 
